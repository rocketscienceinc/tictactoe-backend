...
func NewGameUseCase(logger *slog.Logger, playerRepo playerRepo, gameRepo gameRepo, botUseCase BotUseCase) GameUseCase {
	return &gameUseCase{
...
func (that *gameUseCase) addBotToGame(ctx context.Context, game *entity.Game) error {
	...
	if botMark == entity.PlayerX {
		if err := that.botUseCase.MakeTurn(game); err != nil {
...
func (that *gameUseCase) createGame(ctx context.Context, gameType string, player *entity.Player) (*entity.Game, error) {
